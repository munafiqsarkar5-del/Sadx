...
# HTML TEMPLATE (your complete design)
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- Approval Key Modal (shown to buyer) -->
...
<!-- Orders / Approval Panel (admin only) -->
...
function render(){
  ...
  grid.innerHTML = list.map(p => {
    ...
    return `
    <div class="card ${soldOut ? 'sold-out':''}" data-id="${p.id}">
      ${isAdmin ? `<button class="rm-btn" onclick="removeProduct('${p.id}')">✕</button>` : ''}
      ${isAdmin ? `<button class="so-btn" onclick="toggleSoldOut('${p.id}')" title="Toggle sold out">${soldOut ? '✅' : '🚫'}</button>` : ''}
