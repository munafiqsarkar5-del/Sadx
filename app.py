...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .card .media{width:100%; height:180px; background:#0d0c10; display:flex; align-items:center; justify-content:center; overflow:hidden; position:relative; pointer-events:none;}
  ...
  .card-body{padding:16px; display:flex; flex-direction:column; gap:8px; flex:1; pointer-events:none;}
  ...
  .detail-modal .detail-media{width:100%; max-height:300px; overflow:hidden; border-radius:6px; margin-bottom:14px; background:#0d0c10;}
  .detail-modal .detail-media img, .detail-modal .detail-media video{width:100%; max-height:300px; object-fit:contain;}
  .detail-modal .detail-price{font-family:'JetBrains Mono'; color:var(--gold-bright); font-size:20px; margin:6px 0;}
  .detail-modal .detail-desc{color:var(--muted); font-size:14px; line-height:1.6; margin:8px 0;}
  .detail-modal .detail-thumbs{display:flex; gap:8px; flex-wrap:wrap; margin:10px 0;}
  .detail-modal .detail-thumbs img, .detail-modal .detail-thumbs video{width:80px; height:80px; object-fit:cover; border-radius:4px; border:1px solid var(--line); cursor:pointer;}
  .detail-modal .detail-thumbs img:hover, .detail-modal .detail-thumbs video:hover{opacity:0.8;}
  .detail-modal .detail-key{background:var(--bg); border:1px solid var(--gold); border-radius:6px; padding:12px 14px; margin:12px 0; display:flex; justify-content:space-between; align-items:center;}
  .detail-modal .detail-key .key-text{font-family:'JetBrains Mono'; font-size:18px; letter-spacing:3px; color:var(--gold-bright); font-weight:700;}
  .detail-modal .detail-status{margin:10px 0; padding:8px 14px; border-radius:6px; font-size:13px; font-weight:600; display:inline-block;}
  ...
  .uptime-badge{
    position:fixed; bottom:20px; left:20px; background:rgba(10,10,13,0.85); backdrop-filter:blur(10px);
    border:1px solid var(--line); border-radius:30px; padding:8px 18px; display:flex; align-items:center; gap:10px;
    font-size:11px; font-family:'JetBrains Mono'; color:var(--muted); z-index:55;
    box-shadow:0 4px 20px rgba(0,0,0,0.3);
    transition:0.3s;
  }
  .uptime-badge:hover{border-color:var(--teal);}
  .uptime-dot{
    width:8px; height:8px; border-radius:50%; background:var(--teal); 
    box-shadow:0 0 12px rgba(26,156,140,0.6);
    animation:pulse-dot 2s infinite;
  }
  @keyframes pulse-dot{0%,100%{opacity:1;}50%{opacity:0.4;}}
  .uptime-badge .status-text{color:var(--teal); font-weight:600;}
  ...
  @media (max-width:600px){
    ...
    .uptime-badge{left:12px; bottom:12px; font-size:10px; padding:6px 14px;}
    .uptime-badge .uptime-dot{width:6px; height:6px;}
  }
...
<div class="uptime-badge">
  ...
  <span id="uptimeTime">99.9% Uptime</span>
  <span style="opacity:0.5;">|</span>
...
<div class="overlay" id="detailOverlay">
  <div class="modal detail-modal">
    ...
    <div id="detailKeyBox" class="detail-key" style="display:none;">
      <span class="key-text" id="detailKeyText">— — — —</span>
      <button class="copy-btn" onclick="copyDetailKey()">COPY</button>
    </div>
    <div id="detailActions" class="modal-actions">
      <button class="btn-ghost" onclick="closeDetail()">Close</button>
      <button class="btn-primary" id="detailBuyBtn" onclick="detailBuy()">💬 Buy on WhatsApp</button>
    </div>
  </div>
</div>
...
const WHATSAPP_NUMBER = "03303257478";
...
function updateUptime(){
  ...
  el.textContent = uptime.toFixed(1) + '% Uptime';
}
...
function render(){
  ...
  grid.innerHTML = list.map(p => {
    ...
    if(soldOut){
      ...
      actionBtn = `<button class="buy-btn dl-btn" onclick="event.stopPropagation(); downloadItem('${p.id}')">⬇ DOWNLOAD</button>`;
    } else {
      actionBtn = `<button class="buy-btn" onclick="event.stopPropagation(); buyItem('${p.id}')">💬 BUY ON WHATSAPP</button>`;
    }
    return `
...
function showDetail(id){
  ...
  if(p.images && p.images[0]){
    mediaEl.innerHTML = `<img src="${p.images[0]}" alt="${p.name}">`;
  } else if(p.videos && p.videos[0]){
    mediaEl.innerHTML = `<video src="${p.videos[0]}" controls autoplay muted loop></video>`;
  } else {
    mediaEl.innerHTML = '<span class="mono" style="color:var(--muted);font-size:14px;display:flex;align-items:center;justify-content:center;height:200px;">NO PREVIEW</span>';
  }
  
  document.getElementById('detailPrice').textContent = p.type === 'free' ? 'FREE' : (p.price || 'Contact for price');
  document.getElementById('detailPrice').style.color = p.type === 'free' ? 'var(--teal)' : 'var(--gold-bright)';
  ...
  if(p.images && p.images.length > 1){
    p.images.slice(1).forEach(img => {
      ...
      el.src = img;
      el.onclick = () => { mediaEl.innerHTML = `<img src="${img}">`; };
  ...
  if(p.videos && p.videos.length > 0){
    p.videos.forEach(vid => {
      ...
      el.src = vid; el.muted = true;
      el.onclick = () => { mediaEl.innerHTML = `<video src="${vid}" controls autoplay muted loop></video>`; };
      thumbsEl.appendChild(el);
    });
  }
  ...
  if(p.soldOut){
    ...
    statusEl.textContent = p.type === 'free' ? '✅ Available — Free Download' : '✅ Available — Buy on WhatsApp';
    statusEl.style.color = 'var(--teal)';
    statusEl.style.border = '1px solid var(--teal)';
    statusEl.style.background = 'rgba(26,156,140,0.1)';
    statusEl.style.padding = '8px 14px';
    statusEl.style.borderRadius = '6px';
  }
  
  const keyBox = document.getElementById('detailKeyBox');
  const keyText = document.getElementById('detailKeyText');
  const existingOrder = orders.find(o => o.productId === id && o.status === 'pending');
  if(existingOrder && p.type === 'paid'){
    keyBox.style.display = 'flex';
    keyText.textContent = existingOrder.key;
    currentApprovalKey = existingOrder.key;
  } else {
  ...
  if(p.soldOut){
    ...
    buyBtn.onclick = () => { closeDetail(); downloadItem(p.id); };
    buyBtn.disabled = false;
    buyBtn.style.opacity = '1';
    buyBtn.style.cursor = 'pointer';
  } else {
    buyBtn.textContent = '💬 BUY ON WHATSAPP';
    buyBtn.onclick = () => { closeDetail(); buyItem(p.id); };
    buyBtn.disabled = false;
    buyBtn.style.opacity = '1';
    buyBtn.style.cursor = 'pointer';
  }
...
function copyDetailKey(){
  const key = document.getElementById('detailKeyText').textContent;
  navigator.clipboard.writeText(key).then(()=>{
    const btn = document.querySelector('.detail-key .copy-btn');
...
async function buyItem(id){
  console.log('buyItem called for:', id);
  const p = products.find(x=>x.id===id);
  if(!p) {
...
function resendApprovalWhatsApp(){ 
  if(lastApprovalMsg) {
    openWhatsApp(lastApprovalMsg);
  }
}
...
async function toggleSoldOut(id){
  console.log('toggleSoldOut called for:', id);
  try {
    const res = await fetch(`/api/product/${id}/soldout`, { 
      method: 'PUT',
      headers: {'Content-Type': 'application/json'}
    });
    if(res.ok) {
      const data = await res.json();
      products = data.products || [];
      console.log('Sold out toggled, products:', products.length);
      render();
    } else {
      const err = await res.text();
      console.error('Toggle failed:', err);
      alert('Error toggling sold out: ' + err);
    }
  } catch(e) {
    console.error('Error in toggleSoldOut:', e);
    alert('Error toggling sold out: ' + e.message);
  }
}
...
async function saveProduct(){
  ...
  try {
    ...
    if(res.ok) {
      ...
      const err = await res.text();
      errBox.textContent = 'Error saving product: ' + err;
...
async function removeProduct(id){
  console.log('removeProduct called for:', id);
  ...
  try {
    ...
    if(res.ok) {
      const data = await res.json();
      products = data.products || [];
      console.log('Product removed, products:', products.length);
      render();
    } else {
      const err = await res.text();
...
loadData();
updateOrdersCount();
console.log('Dark Store initialized');
